...
is narrower and more presentation-focused: AgilaEye should work inside a
simulated Facebook-style environment where embedded videos are scanned
repeatedly and SOP performance metrics are visible.
...
- Return deterministic per-video scan results from local TypeScript data.
...
- The codebase keeps a clean simulation module that can be replaced later.
